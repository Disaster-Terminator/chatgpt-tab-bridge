...
| Task | Location | Notes |
|------|----------|-------|
| State machine | `src/extension/core/state-machine.ts` | Phase logic: ready→running→paused→stopped |
| Relay logic | `src/extension/core/relay-core.ts` | Message forwarding between tabs |
| Popup UI | `src/extension/popup.ts` | Control panel |
| Overlay | `src/extension/overlay.css` | In-page floating widget |
| Background | `src/extension/background.ts` | Service worker, message hub |
| Tests | `tests/*.test.mjs` | Node test runner |

## CONVENTIONS (THIS PROJECT)
- TypeScript for source, compiled to JS for extension
- Build output committed to `dist/` (not ignored)
- Tests use Node.js built-in test runner (`node --test`)
- Playwright for e2e browser tests
- No ESLint/Prettier config found

## ANTI-PATTERNS (THIS PROJECT)
- Don't modify `dist/` directly - edit source in `src/`
- `chatgpt.com/` root page not supported as relay target

## COMMANDS
```bash
pnpm run build       # Build extension
pnpm test           # Unit tests
pnpm run test:smoke # Playwright smoke test
pnpm run test:semi  # Semi-auto relay test
pnpm run typecheck  # TypeScript check
```

## NOTES
- Extension uses Manifest V3 (service worker, not background page)
- URL patterns: `chatgpt.com/c/<id>` and `chatgpt.com/g/<project>/c/<id>`
- Protocol: `[BRIDGE_STATE] CONTINUE|FREEZE`尾部标记

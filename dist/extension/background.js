...
function fallbackAdvice(code, severity) {
...
function describeRuntimeIssue(input) {
...
// core/reply-observation-classifier.ts
function classifyReplyObservation(input) {
  if (input.currentHash && input.currentHash !== input.baselineHash) {
    return { kind: "settled", hash: input.currentHash, reason: "assistant_hash_changed" };
  }
  if (input.elapsedMs >= input.hopTimeoutMs) {
    return { kind: "stop", stopReason: STOP_REASONS.HOP_TIMEOUT, reason: "elapsed_timeout" };
  }
  if (input.tabLifecycle?.discarded === true) {
    return { kind: "stop", stopReason: STOP_REASONS.UNREACHABLE_TARGET, reason: "tab_discarded" };
  }
  if (input.tabLifecycle?.frozen === true) {
    return { kind: "continue", progressObserved: false, reason: "tab_frozen_waiting" };
  }
  if (input.pageVisibility === "hidden" && !input.generating && input.replyPending && !input.generationObservedAfterDispatch) {
    return { kind: "stop", stopReason: STOP_REASONS.TARGET_HIDDEN_NO_GENERATION, reason: "hidden_no_generation" };
  }
  if (input.generating) {
    return { kind: "continue", progressObserved: input.idleMs < input.hopTimeoutMs / 2, reason: "generating" };
  }
  return { kind: "continue", progressObserved: false, reason: input.replyPending ? "reply_pending_without_progress" : "awaiting_assistant" };
}
...
async function waitForSettledReply({
  ...
  while (true) {
    ...
    const decision = classifyReplyObservation({
      ...
      pageVisibility: observation.sample.page.hidden ? "hidden" : "visible",
    ...
    if (decision.kind === "stop") {
      return { ok: false, reason: decision.stopReason };
    }
